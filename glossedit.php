...
switch ($action) {
	case "edit" :
		...
		$form  = "<form action=\"";
		$form .= append_sid ($phpbb_root_path."ext/lmdi/gloss/glossedit.".$phpEx, "action=save");
		...
		$form .= "<dl>";
		$form .= "<dl><dt>&nbsp;</dt>";
		$form .= "<dd><input type=\"submit\" name=\"login\" tabindex=\"5\" value=\"$str_regis\" class=\"button1\" /></dd>";
		$form .= "</dl>";
		$form .= "</fieldset>";
		$form .= "</div></div></div>";
		$abc_links = $form;
	...
	case "save" :		// Enregistrement
		...
		$db->sql_query ($sql);
	// Et on dégouline sur l'affichage de la page
	case "rien" :
		...
		$illustration .= "<a href=\"";
		$illustration .= append_sid ($phpbb_root_path."glossedit.".$phpEx, "code=-1&action=edit");
